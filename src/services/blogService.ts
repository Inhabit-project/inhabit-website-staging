...
export const fetchPost = async (
  ...
  try {
    const response = await fetch(
      `${WORDPRESS_API_URL}/wp-json/wp/v2/posts?_embed&include[]=${postId}&lang=${currentLanguage}`
    );
...
const fetchAdjacentPost = async (
  ...
  const response = await fetch(
    `${WORDPRESS_API_URL}/wp-json/wp/v2/posts?${direction}=${date}&per_page=1&order=${order}&_embed&lang=${currentLanguage}`
  );
